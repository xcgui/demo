...
public:
	...
	int OnDraw(HDRAW hDraw,BOOL *pbHandled)
	{
		...
		if (m_nLDownIndex != m_nLMoveIndex)
		{
			//因为选中的开始索引大于结束索引，(光标从右到左) 反之亦然，所以需重新计算
			int nStart = 0;
			int nEnd   = 0;
			if (m_nLDownIndex > m_nLMoveIndex)
			{
				nStart = m_nLMoveIndex;
				nEnd   = m_nLDownIndex;
			}
			else
			{
				nStart = m_nLDownIndex;
				nEnd   = m_nLMoveIndex;
			}

			int nHideWidth = m_memDC.GetTextWidth(m_Text.GetRText(),m_Text.GetVOffset());
			...
			for (int i = m_Text.GetVOffset(); i < m_Text.GetRLen(); i++)
			{
				m_memDC.GetTextSize(m_Text.GetRText()+i,1,&sz);
	...
	int PtInTextIndex(POINT* pPt)
	{
		//如果在，得判断出所在字符串位置索引
		int nLeftWidth = 0;
		int nCharLen   = 0;
		int i = 0;
		for (; i < m_Text.GetVLen(); i++)
		{
			nCharLen = m_memDC.GetTextWidth(m_Text.GetVText()+i,1);
			if ( pPt->x < m_RectText.left + nLeftWidth + nCharLen/2 )
			{
				return i;
			}
			nLeftWidth+=nCharLen;
	...
	int OnLButtonDown(UINT nFlags, POINT *pPt,BOOL *pbHandled)
	{
		...
		m_nLDownIndex = m_Text.GetVOffset() + PtInTextIndex(pPt);
		m_nLMoveIndex = m_nLDownIndex;
//		m_ptDown = *pPt;
		//计算出 鼠标点击位置在 文本的索引
		int nIndex = PtInTextIndex(pPt);
		m_CaretInTextIndex = nIndex;
		int nLeftWidth = m_memDC.GetTextWidth(m_Text.GetVText(),m_CaretInTextIndex);
	...
	int OnMouseMove(UINT nFlags, POINT *pPt, BOOL *pbHandled)
	{
		...
		{
			m_CaretInTextIndex = PtInTextIndex(pPt);
			int nCurrentIndex = m_Text.GetVOffset()+m_CaretInTextIndex;
			if (m_nLMoveIndex != nCurrentIndex)
			{
				m_nLMoveIndex = nCurrentIndex;
				int nLeftWidth = m_memDC.GetTextWidth(m_Text.GetVText(),m_CaretInTextIndex);
				m_Caret.SetPos(m_RectText.left + nLeftWidth,m_RectText.top);
				m_Caret.Show(TRUE);
				XEle_RedrawEle(m_hEle);
