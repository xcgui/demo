...
//绘制文本
		...
		m_memDC.SetDCPenColor(RGB(0,18,0));
		m_memDC.TextOutW(
			m_RectText.left,
			m_RectText.top,
	...
	//以下一个字符为开始 反向计算编辑框可显示宽度
	int GetReverseCalcTextShowOffset()
	{
		int nLeftWidth = 0;
		int nCharWidth = 0;
		int nEdithWidth = m_RectText.right-m_RectText.left;
		int i = m_nTextShowOffset+m_CaretInTextIndex;
		for (; i > 0; i--)
		{
			nCharWidth = m_memDC.GetTextWidth(m_Text.c_str()+i,1);
			if (nLeftWidth+nCharWidth > nEdithWidth)
	...
	int OnEventKeyDown(WPARAM wParam,LPARAM lParam,BOOL *pbHandled)
	{
		...
		case VK_LEFT:
			{
				...
				{
					if (m_nTextShowOffset>2)
					{
						m_nTextShowOffset-=2;
						m_CaretInTextIndex++;
					}else if (m_nTextShowOffset > 1)
					{
						m_nTextShowOffset--;
					}else
					{
		...
		case VK_RIGHT:
			{
				...
				if (m_CaretInTextIndex+m_nTextShowOffset == m_Text.length())
					return 0;//在末尾，直接忽略
		...
		case VK_BACK:
			{
				if (m_nTextShowOffset+m_CaretInTextIndex == 0)
				{
					MessageBeep(-1);
					return 0;
				}
				m_Text.erase(m_Text.begin()+m_nTextShowOffset+m_CaretInTextIndex-1);
