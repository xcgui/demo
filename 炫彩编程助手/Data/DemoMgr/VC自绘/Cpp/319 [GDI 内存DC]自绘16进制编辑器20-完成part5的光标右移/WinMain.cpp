...
public:
	int OnEventKeyDown(WPARAM wParam,LPARAM lParam,BOOL *pbHandled)
	{
		...
		if (wParam == VK_RIGHT)
		{
			...
			if (CaretInfo.m_CaretInWichPart == CARET_IN_PART5)
			{
				...
				if (CarertInfo.m_DataIndex == dwFileSize && CaretInfo.m_TextIndex == 1) 
					return 0;  //在末尾
				...
				{
					int nBufferIndex =CarertInfo.m_DataIndex - m_nScorBarVPos*16;
					m_Part5.GetHexsRect(nBufferIndex,&rt);
					CarertInfo.m_TextIndex = 1;
				...
				}else
				{
					...
					CarertInfo.m_DataIndex++;
					CarertInfo.m_TextIndex = 0;
				...
				{
					//设置当前光标位置
					m_SelectEnd = CarertInfo;
					m_SelectStart = CarertInfo;
				}
			...
			if (CaretInfo.m_CaretInWichPart == CARET_IN_PART6)
			{
				DWORD dwFileSize = 0;
				m_File.GetSizeLessThan4GB(&dwFileSize);
				if (CarertInfo.m_DataIndex == dwFileSize) 
						return 0;  //在末尾
	...
	int OnSize(BOOL *pbHandled)
	{
		//首先调整视图大小
		BorderSize_i bi;
		XSView_GetBorderSize(m_hScrollView,&bi);
		
		//元素控件大小变化 重新计算视图总大小
		LARGE_INTEGER fileSize;
		m_File.GetSizeMoreThan4GB(&fileSize);
		DWORD nRowCount = fileSize.LowPart / 16;
		int nRowHeight = m_PaddingTop+m_nRowHeight+nRowCount*m_nRowHeight+bi.topSize+bi.bottomSize;
		XSView_SetTotalSize(m_hScrollView,1000,nRowHeight);
