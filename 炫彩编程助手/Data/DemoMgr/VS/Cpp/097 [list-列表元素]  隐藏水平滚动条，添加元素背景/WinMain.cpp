...
public:
    ...
    void Init()
    {
        ...
        m_hList= XList_Create(20,40,475,300,m_hWindow);
...
//		XSView_SetBorderSize(m_hList,0,0,0,0);
//		XSView_EnableAutoShowScrollBar(m_hList,FALSE);
		XSView_ShowSBarH(m_hList,FALSE);

//		HELE hSbarH = XSView_GetScrollBarH(m_hList);
//		HBKM hBkInfoM = XEle_GetBkManager(hSbarH);
//		XBkM_AddFill(hBkInfoM,element_state_flag_leave,255,255);

// 
// 		int nWdith = XSView_GetViewWidth(m_hList);
// 		XSView_SetTotalSize(m_hList,270,XSView_GetViewHeight(m_hList));

		hBkInfoM = XEle_GetBkManager(m_hList);
		XBkM_AddFill(hBkInfoM,element_state_flag_leave,RGB(245,245,245),255);

		XEle_RegEventCPP(XList_GetHeaderHELE(m_hList),XE_LIST_HEADER_DRAWITEM,&CMyWindowList::OnListHeaderDrawItem);
