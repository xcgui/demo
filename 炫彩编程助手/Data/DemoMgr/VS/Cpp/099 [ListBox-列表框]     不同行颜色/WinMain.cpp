...
public:
	...
	int OnListBoxTemplateCreateEnd(listBox_item_i* pItem,BOOL *pbHandled)
	{
		...
		HXCGUI hEle = XListBox_GetTemplateObject(m_hListBox,pItem->index,5);
		if (XC_IsHXCGUI(hEle,XC_ELE))
		{
			HBKM hBk = XEle_GetBkManager((HELE)hEle);
			
			if ( (pItem->index%2) == 0)
			{
				if (pItem->nState == list_item_state_leave)
				{
					XBkM_AddFill(hBk,element_state_flag_leave,RGB(128,0,0),50);

				}else if (pItem->nState == list_item_state_stay)
				{
					XBkM_AddFill(hBk,element_state_flag_leave,RGB(128,128,0),100);
				} 
				else
				{
					XBkM_AddFill(hBk,element_state_flag_leave,RGB(128,0,128),150);
				}
			...
			}else
			{
				...
				if (pItem->nState == list_item_state_leave)
				{
					XBkM_AddFill(hBk,element_state_flag_leave,RGB(0,128,0),50);
				...
				{
					XBkM_AddFill(hBk,element_state_flag_leave,RGB(0,128,128),150);
				}
