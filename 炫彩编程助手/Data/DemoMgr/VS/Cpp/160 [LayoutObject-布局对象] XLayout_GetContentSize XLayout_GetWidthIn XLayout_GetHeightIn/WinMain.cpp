...
public:
    ...
    BOOL Create() //创建窗口和按钮
    {
        ...
        if(m_hWindow)
        {
			...
			hEleLeft = XEle_Create(0,0,100,100,m_hWindow);
			XLayout_AddEle(hLayoutBody,hEleLeft);
			XEle_SetLayoutWidth(hEleLeft,layout_size_type_weight,2);
			XEle_SetLayoutHeight(hEleLeft,layout_size_type_fill,1);


			hEleRight = XEle_Create(0,0,100,100,m_hWindow);
			XLayout_AddEle(hLayoutBody,hEleRight);
			XEle_SetLayoutWidth(hEleRight,layout_size_type_weight,3);
			XEle_SetLayoutHeight(hEleRight,layout_size_type_fill,1);


			XLayout_SetPadding(hLayoutBody,20,20,20,20);
	...
	int OnWndSize(UINT nFlags,SIZE *pSize,BOOL *pbHandled)
	{
		...
		//得到真实宽度和高度
		xtrace("wdith-%d height-%d | WidthIn-%d heightIn-%d \r\n",XLayout_GetWidth(hLayout),XLayout_GetHeight(hLayout),
			XLayout_GetWidthIn(hLayout),XLayout_GetHeightIn(hLayout));


		RECT rtEleLeft,rtEleRight;
		XEle_GetClientRect(hEleLeft,&rtEleLeft);
		XEle_GetClientRect(hEleRight,&rtEleRight);
