...
public:
	...
	void Create(HWINDOW hMainWindow)
	{
		...
		m_hWindow = (HWINDOW)XC_LoadLayout(L"弹出窗口\\Layout.xml",m_hMainWindow);

		m_hList = (HELE)XC_GetObjectByID(m_hWindow,999);
		XEle_RegEventCPP(m_hList,XE_LIST_TEMP_CREATE_END,&CRightPane::OnListTemplateCreateEnd);

		m_ComboBoxAutoLogin = (HELE)XC_GetObjectByID(m_hWindow,1001);
		m_ComboBoxVirtualType = (HELE)XC_GetObjectByID(m_hWindow,1002);
		m_ComboBoxVirtualHardWare = (HELE)XC_GetObjectByID(m_hWindow,1003);
		m_ComboBoxBoot = (HELE)XC_GetObjectByID(m_hWindow,1004);
		m_ComboBoxAutoMinSize = (HELE)XC_GetObjectByID(m_hWindow,1005);
		m_ComboBoxIpCount = (HELE)XC_GetObjectByID(m_hWindow,1006);


		m_SelectAll = (HELE)XC_GetObjectByID(m_hWindow,1100);
		m_UnSelectAll = (HELE)XC_GetObjectByID(m_hWindow,1101);
	...
	void AdjustPos()
	{
		...
		MoveWindow(
			...
			rtPaneWindow.bottom - rtPaneWindow.top
			 ,TRUE);
	}
...
private:
	void InitSetting()
	{
 ...
 
		{
			...
			XComboBox_BindAdapter(m_ComboBoxAutoLogin,hAdapter);
...

		{
			...
			HXCGUI hAdapter=XAdTable_Create();
			XComboBox_BindAdapter(m_ComboBoxVirtualType,hAdapter);
			XAdTable_AddColumn(hAdapter,L"name1");
...

		{
			...
			HXCGUI hAdapter=XAdTable_Create();
			XComboBox_BindAdapter(m_ComboBoxVirtualHardWare,hAdapter);
			XAdTable_AddColumn(hAdapter,L"name1");
		...
		}
		{
			...
			HXCGUI hAdapter=XAdTable_Create();
			XComboBox_BindAdapter(m_ComboBoxBoot,hAdapter);
			XAdTable_AddColumn(hAdapter,L"name1");
...

		{
			...
			HXCGUI hAdapter=XAdTable_Create();
			XComboBox_BindAdapter(m_ComboBoxAutoMinSize,hAdapter);
			XAdTable_AddColumn(hAdapter,L"name1");
		...
		}
		{
			...
			HXCGUI hAdapter=XAdTable_Create();
			XComboBox_BindAdapter(m_ComboBoxIpCount,hAdapter);
			XAdTable_AddColumn(hAdapter,L"name1");
        ...
        XAdMap_AddItemText(hAdapterHeader,L"name1",L"选择");
        ...
        for (int i=0;i<20;i++)
        {
			...
			nIndex = XAdTable_AddItemTextEx(hAdapter,L"name1",tmp);;			
...
public:
	void Create()
	{
		...
		m_hStaticText = XC_GetObjectByID(m_hWindow,2000);
		XWnd_AdjustLayout(m_hWindow);
