...
public:
    ...
    void Init()
    {
		...
		m_hImage = XImage_LoadFile(L"a.png",TRUE);
		HELE hEleHeader = XList_GetHeaderHELE(m_hList);
		XEle_RegEventCPP1(hEleHeader,XE_PAINT, &CMyWindowList::OnDraw);
...
//		XEle_RegEventCPP(hEleHeader,XE_LIST_HEADER_DRAWITEM,&CMyWindowList::OnListHeaderDrawItem);
	...
	int OnDraw(HELE hEventEle,HDRAW hDraw,BOOL *pbHandled)
	{
		...
		XDraw_ImageStretch(hDraw,m_hImage,rt.left,rt.top,rt.right,rt.bottom);
		*pbHandled = TRUE;
		return 0;
	}
