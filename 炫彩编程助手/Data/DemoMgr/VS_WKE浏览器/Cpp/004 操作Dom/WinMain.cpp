...
public:
	BOOL Create()
	{
		...
		HELE hBtn = XBtn_Create(8,5,100,20,L"执行js脚本",m_hWindow);
	...
	int OnBtnClick(BOOL *pbHandled)
	{
		...
		wkeJSValue  obJect = wkeJSGlobalObject(pState);
		wkeJSValue  document = wkeJSGet(pState,obJect,"document");
		wkeJSValue  body = wkeJSGet(pState,document,"body");
		wkeJSValue jsvalue = wkeJSGet(pState,body,"outerHTML"); //innerText outerHTML
		
		MessageBoxW(NULL,wkeJSToStringW(pState,jsvalue),L"网页源码",MB_OK);
